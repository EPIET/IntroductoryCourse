...
```{r}
CS(copdata, "case", "class")
```

#### d) group

```{r}
CS(copdata, "case", "group")
```
...
wilcox.test(as.numeric(vealD) ~ case, data = copdata)
```
...
wilcox.test(as.numeric(pastaD) ~ case, data = copdata)
```
...
wilcox.test(as.numeric(champagneD) ~ case, data = copdata)
```
